  ...
  return (
    <div className='flex w-full items-center justify-center min-h-screen p-4 bg-[#ff9f6]'>
        <div className='bg-white rounded-xl shadow-lg w-full max-w-md p-8'>
            ...
            <button type="button" className={`w-full font-semibold py-2 rounded-lg transition duration-200
             bg-[#ff4d2d] text-white hover:bg-[#d43818] cursor-pointer`} onClick={handleSendOtp} disabled={loading}> 
            ...
            <button type="button" className={`w-full font-semibold py-2 rounded-lg transition duration-200
             bg-[#ff4d2d] text-white hover:bg-[#d43818] cursor-pointer`} onClick={handleVerifyOtp} disabled={loading}> 
             { loading ? <ClipLoader size={20} color='white'/> : 'Verify' } 
             </button>
             {err && <p className='text-red-500 text-center my-[10px]'>*{err}</p>}
            </div>}

            {step == 3    //Forgot Password - Enter NewPassword, ConfirmPassword
            &&
            <div>
            <div className='mb-6'>
                ...
                <div className='relative'>
                    <input type={`${showNewPassword ? "text" : "password"}`} className='w-full border-[1px] border-gray-200 rounded-lg px-3 py-2 focus:outline-none'
                        placeholder='Enter New Password' 
                        onChange={(e)=> setNewPassword(e.target.value)} 
                        value={newPassword} required/>
                        <button className='absolute right-5 cursor-pointer top-[14px] text-gray-500'
            ...
            <div className='mb-6'>
                ...
                <div className='relative'>
                    <input type={`${showConfirmPassword ? "text" : "password"}`} className='w-full border-[1px] border-gray-200 rounded-lg px-3 py-2 focus:outline-none'
            ...
            <button type="button" className={`w-full font-semibold py-2 rounded-lg transition duration-200
             bg-[#ff4d2d] text-white hover:bg-[#d43818] cursor-pointer`} onClick={handleResetPassword} disabled={loading}> 
             { loading ? <ClipLoader size={20} color='white'/> : 'ResetPassword' } 
             </button>
             {err && <p className='text-red-500 text-center my-[10px]'>*{err}</p>}
            </div>}
        </div>
