    ...
    fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible">
      ...
      {userData.role == "user" && (
        <div className="md:w-[60%] lg:w-[40%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] hidden md:flex">
          ...
          <div className="w-[80%] flex items-center gap-[10px]">
            <IoSearchSharp size={25} className="text-[#ff4d2d] md:hidden" />
            <input
              type="text"
              placeholder="Search delicious Food..."
              className="px-[10px] text-gray-700 outline-0 w-full"
      ...
      <div className="flex items-center gap-4">
        {userData.role == "user" && (showSearch ? 
          <RxCross2 size={22} className="text-[#ff4d2d] md:hidden" onClick={() => setShowSearch(false)}/>
        :  <IoSearchSharp size={23} className="text-[#ff4d2d] md:hidden" onClick={() => setShowSearch(true)}/>
        )}
        ...
        {myShopData && <> <button className="hidden md:flex items-center gap-1 p-2 cursor-pointer rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d]"
            ...
            <div className="hidden md:flex items-center gap-2 cursor-pointer relative px-3 py-1 rounded-lg bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium">
                ...
                <span className='absolute -right-2 -top-2 text-xs font-bold text-white bg-[#ff4d2d] 
                                 rounded-full px-[10x] py-[1px]'> 0 </span>
            </div>
            ...
            <div className="md:hidden flex items-center gap-2 cursor-pointer relative px-3 py-1 rounded-lg bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium">
                <TbReceiptRupee size={20}/>
                <span className='absolute -right-2 -top-2 text-xs font-bold text-white bg-[#ff4d2d] 
                                 rounded-full px-[8x] py-[1px]'> 0 </span>
            </div>
        </> : (
            <>
        ...
        <div className="w-[40px] h-[40px] rounded-full flex items-center justify-center bg-[#ff4d2d] text-white
                        text-[18px] shadow-xl font-semibold cursor-pointer" onClick={() => setShowInfo((prev) => !prev)}>
                        {userData?.fullName.slice(0, 1)}
        </div>
        {showInfo && (
          <div
            className="fixed top-[80px] right-[10px] md:right-[10%] lg:right-[25%] w-[180px] bg-white shadow-2xl
